...
const DisplayJobexperences = ({ alumniId }) => {
  const [jobExperiences, setJobExperiences] = useState([]);

  useEffect(() => {
    const fetchJobExperiences = async () => {
      try {
        const response = await axios.get(`http://localhost:3000/api/employment-history/${alumniId}`, {
...
export default DisplayJobexperences;
