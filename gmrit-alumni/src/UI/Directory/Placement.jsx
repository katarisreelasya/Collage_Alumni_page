...
import placement2 from '../../assets/Placements-1-PlacementsDay4.jpg';
import Nav from '../Nav';
import TopBar from '../TopBar';
import { useUser } from "../../store/userStore";

const API_URL = 'http://localhost:3000/api/alumni/all/placed';
...
const Student = () => {
  ...
  useEffect(() => {
    const fetchAlumni = async () => {
      ...
      try {
        ...
        if (selectedCompany) queryParams += `&company=${encodeURIComponent(selectedCompany)}`;
    ...
    fetchAlumni();
  }, [searchTerm, selectedYear, selectedBranch, selectedCompany, selectedLocation, sortField, sortOrder, page]);
  ...
  return (
    user ? (
      <>
        <Nav />
        <TopBar img={placement2} name="Placement Directory" />
        <div className="py-8 bg-gray-100">
          <header className="text-center container mx-auto px-4 md:px-6">
            <h1 className="text-3xl md:text-4xl font-semibold text-center mb-6 animate-slideInRight">Placement Directory</h1>
        ...
        <div className="container mx-auto">
          <div className="mb-4 mt-11 justify-center grid grid-cols-2 gap-3 md:grid md:grid-cols-3 md:gap-3 lg:flex lg:space-x-4">
            ...
            <button
              ...
              className="w-[85%] sm:px-4 mx-2 bg-blue-600 text-white rounded-md shadow-md transition-all duration-300 ease-in-out transform hover:scale-105 sm:w-auto"
            >
              Search
            </button>
          </div>
          <div className="mb-4 flex space-x-4 justify-center text-white">
            <button onClick={() => handleSort('name_of_the_alumni')} className="p-2 bg-blue-500 rounded-lg shadow-md transition-all duration-300 ease-in-out transform hover:scale-105">
              Sort by Name {sortField === 'name_of_the_alumni' && (sortOrder === 'asc' ? '↑' : '↓')}
            </button>
            <button onClick={() => handleSort('passed')} className="p-2 bg-blue-500 rounded-lg shadow-md transition-all duration-300 ease-in-out transform hover:scale-105">
              Sort by Year {sortField === 'passed' && (sortOrder === 'asc' ? '↑' : '↓')}
            </button>
            <button onClick={() => handleSort('branch')} className="p-2 bg-blue-500 rounded-lg shadow-md transition-all duration-300 ease-in-out transform hover:scale-105">
          ...
          <div className="flex justify-center mt-4">
            <button className="px-4 py-2 bg-gray-300 mr-2" onClick={() => handlePageChange(page - 1)} disabled={page === 1}>
              Previous
            </button>
            <span>Page {page} of {totalPages}</span>
            <button className="px-4 py-2 bg-gray-300 ml-2" onClick={() => handlePageChange(page + 1)} disabled={page === totalPages}>
              Next
            </button>
          </div>
        </div>
